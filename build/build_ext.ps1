...
$outputZip = "$sourceDir\build\out\EncumbranceTracker.zip"
$localExt = "$sourceDir\build\out\EncumbranceTracker.ext"
...
# Copy to build/out/EncumbranceTracker.ext
Copy-Item $outputZip $localExt -Force

# Install to FGU extensions directory
if (Test-Path $fguExtensionsDir) {
    Write-Host "Installing EncumbranceTracker.ext to FGU ($fguExtensionsDir)..."
    Copy-Item $localExt (Join-Path $fguExtensionsDir "EncumbranceTracker.ext") -Force
}

# Install to FGC extensions directory
if (Test-Path $fgcExtensionsDir) {
    Write-Host "Installing EncumbranceTracker.ext to FGC ($fgcExtensionsDir)..."
    Copy-Item $localExt (Join-Path $fgcExtensionsDir "EncumbranceTracker.ext") -Force
}
